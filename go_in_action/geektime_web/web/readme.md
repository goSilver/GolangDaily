...
### 6.1 处理输入
处理输入要解决的问题：
1. 反序列化输入：将Bogy字节流转换成一个具体的类型
2. 处理表单输入：可以看作是是一个和JSON或者XML差不多的一种特殊序列化方式
3. 处理查询参数：指从URL中的查询参数中读取值，并且转化为对应的类型
4. 处理路径参数：读取路径参数的值，并且转化为具体的类型
5. 重复读取body：http.Request的Body默认只能读取一次，不能重复读取
6. 读取header：从header中读取特定的值，并且转化为对应的类型
7. 模糊读取：按照一定的顺序，尝试从body、header、路径参数或者cookie里面读取值，并且转化为特定类型

### 6.2 处理输出
处理输出要解决的问题：
1. 序列化输出：按照某种特定的格式输出数据，例如JSON或XML
2. 渲染页面：要考虑模版定位、命名和渲染的问题
3. 处理状态码：允许用户返回特定状态码的响应，例如HTTP 404
4. 错误页面：特定HTTP Status或者error的时候，能够重定向到一个错误页面，例如404被重定向到首页
5. 设置Cookie：设置Cookie的值
6. 设置Header：往Header里面放一些东西

### 6.3 考点
1. 能不能重复读取HTTP协议的Body内容？
   1. 原生API是不可以的。但是我们可以自己封装一下，读取body后放在一个地方，后续都从这里读取。
2. 能不能修改HTTP协议的响应？
